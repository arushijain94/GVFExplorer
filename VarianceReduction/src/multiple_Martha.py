...
class TD:
    def __init__(self, mdp, max_steps, num_episodes, S, A, P, size, target_pol, gamma, alpha, reward_alpha,
        ...
        self.val_func = np.zeros((self.n_tar_policies, self.S))  # V(n, s) func
        self.sr_func = np.zeros((self.n_tar_policies, self.S, self.A, self.S))  # sr = (n,s,a,a)
        self.q_predicted = np.zeros((self.n_tar_policies, self.S, self.A))  # sr = (n,s,a,a)
    ...
    def update_behavior_policy_Q(self, s_t, a_t, s_tp1, r):
        ...
        self.Q_behavior[s_t, a_t] += self.alpha * delta_for_beh
...
if __name__ == '__main__':
    ...
    parser.add_argument("--behv_reward_combined", default=0, type=int, help="(0: only weight change in SR,  1: SR+reward weight change)")
    ...
    for iter_ in range(num_iter):
        if iter_ %100 == 0:
            ...
            # td.plot_value(total_env_steps, log_scale=True)
            # td.plot_value_diff(total_env_steps, true_vals)
            # td.plot_value_diff(total_env_steps, true_vals, log_scale=True)
            # td.plot_sr(total_env_steps)
            # td.plot_reward_weight(total_env_steps)
        ...
        # if iter_ % 100 == 0 or (iter_<100 and iter_% 20 ==0):
