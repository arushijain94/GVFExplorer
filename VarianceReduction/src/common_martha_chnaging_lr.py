...
class TD:
    def __init__(self, mdp, max_steps, num_episodes, S, A, P, size, target_pol, gamma, reward_alpha,
        ...
        self.reward_weights = np.zeros((self.n_tar_policies, self.S, self.A))
    ...
    def update_sr_func(self, s_t, a_t, s_tp1, r_t, combined_R_SF_weight_change=False, intrinsic_reward=0):
        ...
        reward_delta = r_t - self.reward_weights[:,s_t,a_t]
        self.reward_weights[:,s_t,a_t] += self.curr_alpha * reward_delta
    ...
    # def update_val_target(self):
    #     self.val_func = np.einsum('nsat,nt->nsa', self.sr_func, np.mean(self.reward_weights,axis=-1))
    #     # self.val_func = np.einsum('nsa,nsa->ns', self.target_pol, self.q_predicted)
...
if __name__ == '__main__':
    ...
    wandb_dir = f"{args.wandb_group}_{current_time}_{args.min_alpha}_{args.lr_decay_steps}_{args.type_behavior_policy}_{args.behv_reward_combined}_{args.seed}"
