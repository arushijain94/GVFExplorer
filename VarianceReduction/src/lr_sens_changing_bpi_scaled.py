...
if __name__ == '__main__':
    ...
    parser.add_argument("--n_target_pol", default=4, type=int, help="Num of Target Policies")
    ...
    parser.add_argument("--last_k", default=20, type=int, help="last k values for moving reward func")
    parser.add_argument("--exploration_steps", default=4000, type=int, help="exploration steps")
    ...
    parser.add_argument("--num_reward_obj", default=5, type=int, help="num of diff reward fucntions")
        ...
        for j in range(len(lr_q_list)):
            ...
            for iter_ in range(num_iter):
                ...
                if total_env_steps > 1000000:
                    ...
                    wandb.log(metrics)
                    print(
                        f" here ----- alphaVar: {min_alpha_var}, alphaQ {min_alpha_Q}, avg_mse: {mse_val} at samples: {total_env_steps}")
                    break
    ...
    for i in range(len(lr_var_list)):
        for j in range(len(lr_q_list)):
            print(f"LR_VAR: {lr_var_list[i]}, LR_Q {lr_q_list[j] }, avg_mse: {mse_value_list[(lr_var_list[i],lr_q_list[j])]},samples: {num_samples_consumed[(lr_var_list[i],lr_q_list[j])]}")
