...
class TD:
    def __init__(self, mdp, max_steps, num_episodes, S, A, P, size, target_pol, gamma, max_alpha_Q, max_alpha_var, min_alpha_Q,
        ...
        self.var_func = np.ones((self.n_tar_policies, self.S, self.A))*0.5  # var(s,a)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_alpha_Q", default=1, type=float, help="Learning Rate")
    parser.add_argument("--min_alpha_Q", default=0.05, type=float, help="Learning Rate")
    parser.add_argument("--lr_decay_steps_Q", default=500000, type=int, help="Learning Rate decay in these many steps")
    parser.add_argument("--max_alpha_var", default=1, type=float, help="Learning Rate")
    parser.add_argument("--min_alpha_var", default=0.05, type=float, help="Learning Rate")
    parser.add_argument("--lr_decay_steps_var", default=500000, type=int, help="Learning Rate decay in these many steps")
    ...
    wandb_dir = f"{args.wandb_group}_{current_time}_{args.min_alpha_Q}_{args.lr_decay_steps_Q}"
    ...
    lr_var_list = [0.1, 0.25, 0.5, 0.8, 0.9, 0.95]
    lr_q_list = [0.1, 0.25, 0.5, 0.8, 0.9, 0.95]
    mse_value_list = {}
    num_samples_consumed= {}
    mse_dict =defaultdict(list) # (lr_q, lr_var)
    for i in range(len(lr_var_list)):
        for j in range(len(lr_q_list)):
            min_alpha_var = lr_var_list[i]
            min_alpha_Q = lr_q_list[j]
            metrics = {}
            ...
            for iter_ in range(num_iter):
                ...
                if total_env_steps > 500000:
                    ...
                    mse_val = np.mean(mse)
                    mse_value_list[(min_alpha_var, min_alpha_Q)] = mse_val
                    num_samples_consumed[(min_alpha_var, min_alpha_Q)] = total_env_steps
                    mse_dict[(min_alpha_Q, min_alpha_var)].append(mse_val)
                    metrics["LR_Q"] = min_alpha_Q
                    metrics["LR_Var"] = min_alpha_var
                    metrics["mse_avg"] = mse_val
                    metrics["num_samples"] = total_env_steps
                    wandb.log(metrics)
                    print(f" here ----- alphaVar: {min_alpha_var}, alphaQ {min_alpha_Q}, avg_mse: {mse_val}")
                    break

    # averaged_mse_dict = {}
    # for i in range(len(lr_var_list)):
    #     for j in range(len(lr_q_list)):
    #         average_mse = np.mean(mse_dict[(j,i)])
    #         averaged_mse_dict[(j,i)] = average_mse
    #         print(f"LR_VAR: {lr_var_list[i]}, LR_Q: {lr_q_list[j] }, avg_mse: {average_mse}")

    # pprint.pprint(averaged_mse_dict)
