...
from src.common import LinearScheduleClass, get_target_policies
import numpy as np
...
class TD:
    def __init__(self, mdp, max_steps, num_episodes, S, A, P, size, target_pol, gamma, alpha, rho_min, rho_max,
                 n_tar_policies, exploration_steps, temperature_steps, type_behavior_policy, exp_next_val,
                 online_alpha=0.05, val_func=None, var_func=None):
        self.mdp = mdp
        self.env = mdp.env
        ...
        if var_func == None:
            self.var_func = np.zeros((self.n_tar_policies, self.S, self.A))  # var(s,a)
    ...
    def get_reward(self):
        ...
        R,_ = common.create_multiple_reward(self.n_tar_policies, self.size,
    ...
    def get_mean_reward(self):
        if self.env_name == "tmaze":
            ...
            self.mean_R[3, 34, :] = 1
        elif self.env_name == "simple_grid" or self.env_name == "gvfs_grid":
    ...
    def update(self, behavior_policy, total_steps):
        ...
        for e in range(self.num_episodes):
            ...
            while not done and env_steps < self.max_steps:
                ...
                step += 1
                s_a_s_next_r = (int(s), int(a), int(s_next), r[:,s,a])
        ...
        # normalized_var = self.function_normalization()
        # self.update_var_max()
    ...
    def plot_mat(self, mat, num_steps, title):
        ...
        # add the goals
        circle = patches.Circle((self.size - 1, self.size - 1), radius=0.4, edgecolor='green',
                                  facecolor='green',label='Goals')
        ax.add_patch(circle)
        circle = patches.Circle((self.size, self.size - 1 - self.size), radius=0.4, edgecolor='green',
                                facecolor='green', label='Goals')
        ax.add_patch(circle)
    ...
    def plot_state_visitation(self, num_episodes, policy, num_steps):
        ...
        return self.plot_mat(state_visitation, num_steps, 'Visitation Frequency')
    ...
    def plot_value(self, num_steps):
        var_mat = np.mean(self.get_val(),axis=-1)[:,:-1] # [n s]
        var_mat = var_mat.reshape((self.n_tar_policies, self.size, self.size))
        for i in range(self.n_tar_policies):
            plt = self.plot_mat(var_mat[i], num_steps, f"value_{i}")
            wandb.log({"num_samples": num_steps,
                       f"Value_{i}": wandb.Image(plt)})
            plt.close()
        plt = self.plot_mat(np.mean(var_mat, axis=0), num_steps, "MeanValue")
        wandb.log({"num_samples": num_steps,
                   f"MeanValue": wandb.Image(plt)})
        plt.close()

    def plot_value_diff(self, num_steps, true_val):
        true_val = true_val[:, :-1]
        var_mat = np.mean(self.get_val(), axis=-1)[:, :-1]  # [n s]
        abs_diff = np.abs(true_val - var_mat)
        abs_diff = abs_diff.reshape((self.n_tar_policies, self.size, self.size))
        for i in range(self.n_tar_policies):
            plt = self.plot_mat(abs_diff[i], num_steps, f"diff_value_{i}")
            wandb.log({"num_samples": num_steps,
                       f"diff_value_{i}": wandb.Image(plt)})
            plt.close()
        plt = self.plot_mat(np.mean(abs_diff, axis=0), num_steps, "Mean_diff_value")
        wandb.log({"num_samples": num_steps,
                   f"Mean_diff_value": wandb.Image(plt)})
        plt.close()
...
def get_true_vals(mdp, n_target_pol, S, target_policies, R, gamma):
    ...
    for i in range(n_target_pol):
        ...
        # print(f"R_{i} is {R[i]} and V_{i} is {true_vals[i]}")
    return true_vals
...
if __name__ == '__main__':
    ...
    parser.add_argument("--wandb_group", default="Grid_BPI", type=str, help="Group name for Wandb")
    ...
    parser.add_argument("--tar_pol_loc", default="/home/mila/j/jainarus/scratch/VarReduction/Det_10/ideal_targets",
    ...
    parser.add_argument("--env_name", default="gvfs_grid", type=str, help="{simple_grid, gvfs_grid}")
    parser.add_argument("--type_obj", default=1, type=int, help="{1:var_sum, 2:norm_var_sum, 3:var_weighted_sum, 4:norm_var_weighted_sum}")
    parser.add_argument("--last_k", default=10, type=int, help="last k values for moving reward func")
    ...
    wandb_dir = f"{args.wandb_group}_{current_time}"
    ...
    mdp = common.create_env(env_name, size=args.size, gamma=args.gamma, stochastic_transition=args.stochastic_env)
    num_states = mdp.S
    num_actions = mdp.A
    gamma = mdp.gamma
    env = mdp.env
    ...
    # target_policies = np.array(target_policies) # [n s a] dim
    target_policies = get_target_policies(num_states, env_name, args.n_target_pol)
    ...
    true_vals = get_true_vals(mdp, n_target_pol, num_states, target_policies, td.get_mean_reward(), gamma)
    # normalized_weights = (1.0 / n_target_pol) * np.ones((n_target_pol, num_states, num_actions))
    ...
    for iter_ in range(num_iter):
        if iter_ %50 == 0:
            ...
            # w_metrics = np.mean(normalized_weights, axis=1)

            metrics = {}
            for i in range(n_target_pol):
                metrics["mse_" + str(i + 1)] = mse_metrics[i]
                metrics["bias_" + str(i + 1)] = bias_metrics[i]
                # metrics["w_" + str(i + 1)] = w_metrics[i]
                # metrics["trueval_" + str(i + 1)] = true_vals_metrics[i]

            metrics["mse_avg"] = np.mean(mse_metrics)
            metrics["bias_avg"] = np.mean(bias_metrics)
            # metrics["true_target_val"] = np.mean(true_vals_metrics)
            ...
            # plot variance
            td.plot_var(total_env_steps)
            td.plot_value(total_env_steps)
            td.plot_value_diff(total_env_steps, true_vals)
        ...
        if iter_ % 50 == 0 or (iter_<60 and iter_% 30 ==0):
            # print("iter:", iter_)
