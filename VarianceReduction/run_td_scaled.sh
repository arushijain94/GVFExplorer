...
min_alpha=$1
seed=$2
type_target=$3 # Rb, Mixture
temp_name=$4
num_iter=$5
group_name=TDScaled_${temp_name}_10Reward_New
name='TDScale'
logs_name=${name}_targettype${type_target}

python -m src.multiple_target_td_scaled_exp --n_target_pol ${n_target_pol} --num_reward_obj ${num_reward_obj} --size ${size} --lr_decay_steps ${lr_decay_steps} --max_alpha ${max_alpha} --min_alpha ${min_alpha} --env_name ${env_name} --last_k ${last_k} --exploration_steps ${exploration_steps} --seed ${seed} --type_target ${type_target} --stochastic_env ${stochastic_env} --wandb_group $group_name --num_episodes ${num_ep} --num_iter ${num_iter} --timeout ${timeout} --logdir ${logs_base_dir}/TD_Scaled/${logs_name}/seed_${seed}