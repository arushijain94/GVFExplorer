...
group_name=Martha_singleR_greedy_pol_1
num_iter=9000
temperature_steps=400
size=5
env_name=semigreedy_online_rooms_same_reward_grid
...
python -m src.multiple_Martha --behv_reward_combined ${behv_reward_combined} --env_name ${env_name} --exp_next_val ${exp_next_val} --temperature_steps ${temperature_steps} --type_behavior_policy ${type_behavior_policy} --last_k ${last_k} --type_obj ${type_obj} --exploration_steps ${exploration_steps} --num_episodes ${num_ep} --random_start_beh ${random_start_beh} --seed ${seed} --size ${size} --n_target_pol ${n_target_pol} --beh_min ${beh_min} --stochastic_env ${stochastic_env} --wandb_group $group_name --wandb_name $wandb_name --num_episodes ${num_ep} --num_iter ${num_iter} --timeout ${timeout} --alpha ${alpha} --logdir ${logs_base_dir}/SR/${logs_name}/seed_${seed}
