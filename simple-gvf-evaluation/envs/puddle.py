...
class PuddleSimpleEnv(gym.Env):
    ...
    def __init__(self, goals=[[0.95, 0.95], [0.4, 0.65]],
        ...
        super(PuddleSimpleEnv, self).__init__()
        ...
        self.noise = noise
        self.thrust = thrust
        self.puddle_reward_std = puddle_reward_std
    ...
    def step(self, action):
        ...
        for i in range(self.num_goals):
            if np.linalg.norm(self.pos - self.goals[i]) <= self.goal_threshold:
                rewards[i] += 50.
                reached_goal = True

        if self.is_in_puddle(self.pos) and reached_goal == False:
            rewards += np.random.normal(loc=0.0, scale=self.puddle_reward_std, size=rewards.shape)
            # print("rewards in puddle:", self.rewards)
        # print("reward:", rewards)

        terminate = False
        truncate = False
        if self._current_episode_step >= self._max_episode_steps:
            truncate=True
        # if any(np.linalg.norm(self.pos - goal) < self.goal_threshold for goal in self.goals):
        #     terminate = True

        # done = np.linalg.norm(
        #     self.pos - self.goal) < self.goal_threshold or self._current_episode_step >= self._max_episode_steps

        return self.pos.copy(), rewards, False, False, {}
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        while True:
            ...
            if not any(np.linalg.norm(self.pos - goal) < self.goal_threshold for goal in self.goals):
                break
    ...
    def is_in_puddle(self, pos):
        for center, width in zip(self.puddle_center, self.puddle_width):
            lower_bounds = center - width / 2
            upper_bounds = center + width / 2
            if np.all((pos >= lower_bounds) & (pos <= upper_bounds)):
                return True
        return False
    ...
    def plot_trajectory(self, trajectory):
        ...
        # Add each puddle as a rectangle
        for center, width in zip(self.puddle_center, self.puddle_width):
            width = width/2
            lower_left_corner = (center[0] - width[0], center[1] - width[1])
            rect = patches.Rectangle(lower_left_corner, width[0] * 2, width[1] * 2, linewidth=1, edgecolor='blue',
