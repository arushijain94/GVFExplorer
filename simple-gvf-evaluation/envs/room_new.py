...
class SingleGoalRoomEnv(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, goals=[[0.05, 0.95]],
                 goal_threshold=0.05,
                 noise=0.005,
                 thrust=0.025,
                 std_reward=10.):
        super(SingleGoalRoomEnv, self).__init__()
    ...
    def step(self, action):
        ...
        for i in range(self.num_goals):
            if np.linalg.norm(self.pos - self.goals[i], ord=1) <= self.goal_threshold:
                rewards[i] = np.random.normal(loc=100.0, scale=self.std_reward)
                terminate = True
