...
class VariancePolicyEvaluationAgent:
    ...
    def select_behavior_action(self, obs, epsilon, var_lambda, temperature):
        ...
        if random.random() < epsilon:
            ...
            with torch.no_grad():
                ...
                # print(f"beh prob shape:{behavior_policy_prob.shape}, prob: {behavior_policy_prob}")
    ...
    def plot_variance_func(self, global_step, normalized_counts=False, v_min=-1, v_max=5, use_v_limit=False):
        var_mat = self.get_discretized_value_mat(self.var_network, bins=20)
        for i in range(self.reward_dim):
            plot_matrix(self.eval_env, self.writer, var_mat[i], f"Variance_{i+1}", global_step, normalized_counts=normalized_counts, v_min=v_min, v_max=v_max, use_v_limit=use_v_limit)
        plot_matrix(self.eval_env, self.writer, np.mean(var_mat, axis=0), "MeanVariance", global_step, normalized_counts=normalized_counts, v_min=v_min, v_max=v_max, use_v_limit=use_v_limit)

    def plot_variance_func_log(self, global_step,v_min=-1, v_max=5, use_v_limit=False, normalized_counts=False):
        var_mat = self.get_discretized_value_mat(self.var_network, bins=20)
        for i in range(self.reward_dim):
            plot_matrix(self.eval_env, self.writer, var_mat[i], f"LogVariance_{i+1}", global_step, normalized_counts=normalized_counts, log_value=True, v_min=v_min, v_max=v_max, use_v_limit=use_v_limit)
        plot_matrix(self.eval_env, self.writer, np.mean(var_mat, axis=0), "LogMeanVariance", global_step, normalized_counts=normalized_counts, log_value=True, v_min=v_min, v_max=v_max, use_v_limit=use_v_limit)

    # def plot_variance_func_log(self, global_step):
    #     var_mat = self.get_discretized_value_mat(self.var_network, bins=20)
    #     for i in range(self.reward_dim):
    #         plot_matrix(self.eval_env, self.writer, var_mat[i], f"LogVariance_{i+1}", global_step, normalized_counts=False, log_value=True)
    #     plot_matrix(self.eval_env, self.writer, np.mean(var_mat, axis=0), "LogMeanVariance", global_step, normalized_counts=False, log_value=True)
...
def load_target_policy_and_state_val_dict(args):
    ...
    locs = [loc_1, loc_2] #[loc_1, loc_2, loc_3]
    target_agents = []
    state_to_vals = []
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.beh_plot_frequency == 0:
                ...
                # running_agent.get_state_visitation_beh_policy(global_step, bins=20, num_trajectories=3000)
                # plot var
                # running_agent.plot_variance_func(global_step)
                running_agent.plot_variance_func(global_step, normalized_counts=False, v_min=0, v_max=500, use_v_limit=True)
                running_agent.plot_variance_func_log(global_step, v_min=-1, v_max=5, use_v_limit=True, normalized_counts=False)
                # running_agent.plot_variance_func_log(global_step)
