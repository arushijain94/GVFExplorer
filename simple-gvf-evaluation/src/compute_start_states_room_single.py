...
def get_strategic_start_states(num_samples=1, bins=20):
    start_states = []
    bin_size = 1./bins
...
def submit_sbatch_job(initial_state):
    ...
    command = " ".join(["python", "-m", "src.true_policy_evaluation_rooms", "--start-state", initial_state_str,
                        "--config", "./hyperparams/simple_dqn_single_goal_room.yml"])
    print("command:", command)
    # # call sbatch script
    ok = subprocess.call(["sbatch", "mc_torchy2.sh", command])
    time.sleep(0.025)
...
if __name__ == "__main__":
    ...
    # Simple Target Policy

    # get random states:
    random_states = get_strategic_start_states()
