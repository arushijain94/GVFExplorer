...
class PolicyEvaluationAgent:
    ...
    def __init__(self, args, target_policy_agents):
        ...
        # reward linear regression network
        self.cumulant_model = LinearRegressionNet(self.reward_dim, self.reward_dim)
        self.cumulant_optimizer = optim.Adam(self.cumulant_model.parameters(), lr=self.args.learning_rate)
    ...
    def update_sf_and_cumulant_weights(self, global_step, data):
        ...
        with torch.no_grad():
            reward_features = self.get_reward_feature(data.next_observations) # [batch size * reward dim]
            # print("reward feature dime: ", reward_features.shape)
            ...
            sf_target = reward_features + args.gamma * torch.einsum('n,nr->nr', curr_dones, sf_next_values)
            # sf_target = reward_features + args.gamma * torch.einsum('n,nr->nr', curr_dones, sf_next_values)
            # print(" sf target shape:", sf_target.shape)
        ...
        ## update cumulant model

        # Store initial weights
        initial_cumulant_weights = copy.deepcopy(self.cumulant_model.linear.weight.data)
        cumulant_loss = F.mse_loss(data.rewards, self.cumulant_model(reward_features))
        # optimize the model
        self.cumulant_optimizer.zero_grad()
        cumulant_loss.backward()
        self.cumulant_optimizer.step()

        # Calculate weight change (L1 norm)
        total_l1_norm_weight_change += torch.abs(
            self.cumulant_model.linear.weight.data - initial_cumulant_weights).sum()
    ...
    def get_estimated_q_from_sf(self, state_tensor):
        cumulant_weights = self.get_cumulant_model_weights() # [reward_dim * reward_dim]
        with torch.no_grad():
            sf_val = self.sf_net(state_tensor).to(self.device) # [batch size * a * reward dim]

        Q_values = torch.matmul(sf_val, cumulant_weights.t())
        # Q_values = torch.einsum('nar,rl-> nal', sf_val, cumulant_weights) # Q = <sf, w>
        return Q_values
